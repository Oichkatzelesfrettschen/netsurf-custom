...
bool html_exec(struct content *c, const char *src, size_t srclen)
{
	...
	if (htmlc->jsthread == NULL) {
		/* Ensure a JS context exists */
		union content_msg_data msg_data;
		msg_data.jsthread = &htmlc->jsthread;
		content_broadcast(&htmlc->base, CONTENT_MSG_GETTHREAD,
				  &msg_data);
		if (htmlc->jsthread == NULL) {
			NSLOG(netsurf, DEEPDEBUG,
			      "Unable to exec, no JS thread");
			return false;
		}
	}

	/* Execute directly via the JS engine rather than inserting a
	 * <script> element into the DOM.  The old approach fired
	 * DOMNodeInserted mutation events, which set libdom's
	 * dispatching_mutation semaphore.  While that semaphore is
	 * non-zero every DOM modification from within the executing
	 * script returns DOM_NO_MODIFICATION_ALLOWED_ERR, making
	 * createElement/appendChild/setAttribute etc. silently fail.
	 * Direct execution avoids the mutation-event path entirely. */
	return js_exec(htmlc->jsthread, (const uint8_t *)src, srclen,
		       "html_exec");
}
